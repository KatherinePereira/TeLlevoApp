...
export class ViajePage implements OnInit, AfterViewInit, OnDestroy {
  patente: string = '';
  viaje: any;
  map!: mapboxgl.Map;
  ...
  private static readonly TOLERANCIA_UBICACION = 0.0001; 
  ...
  ngOnDestroy() {
    ...
    this.ubicacionService.setConductorActivo(false); // Asegura marcar conductor como inactivo
  }
    ...
    fetch(url)
      ...
      .then((data) => {
        if (data.routes && data.routes.length > 0) {
          ...
          this.mensajes.show('No se pudo obtener la ruta.');
        }
      })
      .catch((error) => {
        console.error('Error al obtener la ruta:', error);
        this.mensajes.show('Error al obtener la ruta. Intenta de nuevo más tarde.');
      });
  }

  unirseAlViaje() {
    console.log('Intentando unirse al viaje:', this.viaje);

    // Verifica si el viaje es válido y si hay asientos disponibles
    if (!this.viaje || parseInt(this.viaje.numAsientos) <= 0) {
        this.mensajes.show('No hay asientos disponibles.');
        return Promise.reject('No hay asientos disponibles.');
    }

    const nuevaCapacidad = parseInt(this.viaje.numAsientos) - 1;

    return this.viajesService.actualizarCapacidadViaje(this.viaje.viajeId, nuevaCapacidad)
        .then(() => {
            // Si la nueva capacidad es 0, actualiza viajeConCupo a false
            if (nuevaCapacidad === 0) {
                return this.viajesService.actualizarViajeConCupo(this.viaje.viajeId, false)
                    .then(() => {
                        this.mensajes.show('El viaje está completo.');
                        this.router.navigate(['/su-viaje', this.patente]);
                    })
                    .catch(error => {
                        this.mensajes.show('Error al actualizar el estado del viaje.');
                        console.error('Error al actualizar el estado del viaje:', error);
                        return Promise.reject(error);
                    });
            } else {
                this.mensajes.show('Te has unido al viaje, quedan ' + nuevaCapacidad + ' asientos.');
                this.router.navigate(['/su-viaje', this.patente]);
                return Promise.resolve();
            }
        })
        .catch((error) => {
            this.mensajes.show('Error al unirse al viaje.');
